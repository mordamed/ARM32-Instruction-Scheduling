...
model side-channel leakage as a probabilistic event using a Bayesian Network
(inspired by Lecture 7). Side-channel vulnerability is not binary; the risk
  ...
  \resizebox{\columnwidth}{!}{%
    \begin{tabular}{llcccc}
      ...
      \multirow{3}{*}{10}
          ...
          & MDP      & 10.3          & 0.3  & 1.40       & 0.002         \\
      \midrule
      \multirow{3}{*}{30}
          & Bayesian & 31.7          & 1.7  & 0.12       & 0.084         \\
          & CSP      & \textbf{30.0} & 0.0  & 0.09       & 0.858         \\
          & MDP      & 30.7          & 0.7  & 4.10       & 0.005         \\
      \midrule
      \multirow{3}{*}{50}
          & Bayesian & 53.0          & 3.0  & 0.20       & 0.516         \\
          & CSP      & \textbf{50.0} & 0.0  & 0.15       & 1.789         \\
          & MDP      & 50.7          & 0.7  & 8.50       & 0.020         \\
      \bottomrule
    \end{tabular}%
  }
\end{table}

%% TODO: rerun experiments/run_all.py --k 50 to regenerate actual Bayesian numbers
%% and replace the preliminary values below. Current benchmark_results.csv
%% only contains k=3 runs for bayesian/csp/mdp (no A* solver exists in code).
\begin{table}[t]
  \centering
  \caption{%
    Benchmark results at $k=50$ (high-security regime; preliminary).
    Larger $k$ forces longer inter-share separations, increasing makespan.
    Numbers to be refreshed once the $k=50$ benchmark run completes.
  }
...
count grows).  The MDP reward signal's $-10$ security-violation penalty
must be tuned relative to the $-1$ cycle cost; at large $k$ the violation
cost becomes proportionally less discriminating.
...
optimal combinatorial solver; (2)~Bayesian Inference offers a much more nuanced
trade-off than a rigorous threshold, successfully balancing statistical leakage risk
with code density; (3)~DQN provides the fastest \emph{inference} after training but
...
oscilloscopes, transfer learning for the MDP across different block distributions,
and a hybrid CSP\,+\,RL approach where the CSP provides a feasibility oracle.
